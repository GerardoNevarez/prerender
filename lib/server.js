...
server.onPhantomCreate = function(phantom) {
    ...
    this.phantom.on('unexpectedExit', function(err) {
        ...
        phridge.disposeAll().then(restartPhantom).catch(function(err) {
            util.log('error disposing phantomjs', err);
            restartPhantom();
        });
    });
};
...
server.exit = function() {
    ...
    phridge.disposeAll().then(terminatePhantom).catch(function(err) {
        util.log('error disposing all phantomjs instances:', err);
        terminatePhantom();
    });
};
